...
if archivos_csv:
    ...
    archivo_seleccionado_nombre = st.selectbox(
        "Selecciona la sesión de simulación/ejecución:",
        nombres_limpios
    )
    ...
    if len(df) > 0:
        ...
        st.info(
            "Esta sesión acaba de iniciar y aún no tiene alertas registradas. "
            "Prueba cerrando los ojos frente a la cámara."
        )

else:
    st.warning(
        "No se han encontrado archivos de datos. "
        "Inicia el servidor primero para generar el archivo de ejecución."
    )